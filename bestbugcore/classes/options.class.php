	...
	class BestBug_Core_Options {
        ...
        public function options() {
    		...
    		foreach ($this->options as $key => $option) {
				...
				$this->type = isset($option['type']) ? $option['type'] : '';
		...
		public function options_fields(){
			...
			foreach ($fields as $key => $field) {
				...
				if(in_array($field['type'], $allow)) {
					...
					esc_html_e('Type-field is not exists', 'hmh-woo-product-bundles');
				}
			}
		...
		public function end_form_html(){
			?>
					...
					<button type="submit" name="submit" class="button success">
						...
						<?php echo (!empty($this->button_text))?$this->button_text:esc_html__('Save Changes', 'hmh-woo-product-bundles') ?>
					</button>
				</form>
			</div>
			<?php
		...
		public function save_post(){
			...
			if(isset($_POST['post_title']) && !empty($_POST['post_title'])) {
				$post['post_title'] = sanitize_text_or_array_field($_POST['post_title']);
				unset($_POST['post_title']);
			}
			if(isset($_POST['post_name']) && !empty($_POST['post_name'])) {
				$post['post_name'] = sanitize_text_or_array_field($_POST['post_name']);
				unset($_POST['post_name']);
				
				$path = $post['post_name']; 
				$post_type = 'page';
				if(isset($_POST['post_type']) && !empty($_POST['post_type'])) {
					$post_type = sanitize_text_or_array_field($_POST['post_type']);
				}
			...
			if(isset($_POST['post_type']) && !empty($_POST['post_type'])) {
				$post['post_type'] = sanitize_text_or_array_field($_POST['post_type']);
				unset($_POST['post_type']);
			}
			if(isset($_POST['post_status']) && !empty($_POST['post_status'])) {
				$post['post_status'] = sanitize_text_or_array_field($_POST['post_status']);
				unset($_POST['post_status']);
			}
			if(isset($_POST['post_content']) && !empty($_POST['post_content'])) {
				$post['post_content'] = sanitize_text_or_array_field($_POST['post_content']);
				unset($_POST['post_content']);
			}
			if(isset($_POST['ID']) && !empty($_POST['ID'])) {
				$post['ID'] = sanitize_text_or_array_field($_POST['ID']);
				unset($_POST['ID']);
			}
